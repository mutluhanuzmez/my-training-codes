...
function compare(a, b) {
    const a_time = a.duration;
    const b_time = b.duration;
    if (a_time < b_time) {
        return -1;
    }
    if (a_time > b_time) {
...
function compareContributors(a, b) {
    const a_time = a.qualityOfContributor;
    const b_time = b.qualityOfContributor;
    if (a_time < b_time) {
        return 1;
    }
    if (a_time > b_time) {
    ...
    constructor(problemLines) {
        ...
        for (let i = 0; i < this.numberOfContributors; i++) {
            ...
            for (let j = 0; j < contributor.numberOfSkills; j++) {
                ...
                contributor.qualityOfContributor += skill.level;
        ...
        this.contributors.sort(compareContributors);
    ...
    solver() {
        ...
        for (let i = 0; i < this.projects.length; i++) {
            ...
            for (let j = 0; j < currentProject.numberOfRequiredSkills; j++) {
                ...
                let findC = this.findAvailableContributor(currentSkill, currentProject.bestBefore, this.projects[i].assignedContributorsSet);
                if (findC == -1) {
                    skillCheck = false;
                    break;
                } else {
                    if (currentSkill.level >= this.contributors[findC].level) {
                        ...
                        this.contributors[findC].qualityOfContributor += 1;
                    }
    ...
    findAvailableContributor(skill, worstIdealStartTime, assignedContributorsSet) {
        ...
        for (let i = 0; i < this.contributors.length; i++) {
            ...
            if (!assignedContributorsSet.has(i)) {
                if (contributor.availableTime <= worstIdealStartTime-1) {
                    ...
                    let assignedContributors = Array.from(assignedContributorsSet.keys());
                    for (let j = 0; j < assignedContributors.length; j++) {
                        ...
                        if (mentorAdequate) {
                            for (let z = 0; z < contributor.skills.length; z++) {
                                ...
                                let dif = skill.level - cSkill.level;
